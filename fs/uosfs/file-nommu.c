...
static int uosfs_nommu_setattr(struct mnt_idmap *, struct dentry *, struct iattr *);
static unsigned long uosfs_nommu_get_unmapped_area(struct file *file,
						   unsigned long addr,
						   unsigned long len,
						   unsigned long pgoff,
						   unsigned long flags);
static int uosfs_nommu_mmap(struct file *file, struct vm_area_struct *vma);

static unsigned uosfs_mmap_capabilities(struct file *file)
{
...
const struct file_operations uosfs_file_operations = {
	.mmap_capabilities	= uosfs_mmap_capabilities,
	.mmap			= uosfs_nommu_mmap,
	.get_unmapped_area	= uosfs_nommu_get_unmapped_area,
 ...
 * memory
 */
int uosfs_nommu_expand_for_mapping(struct inode *inode, size_t newsize)
{
 ...
 *
 */
static int uosfs_nommu_resize(struct inode *inode, loff_t newsize, loff_t size)
{
	int ret;
	...
	if (size == 0) {
		...
		return uosfs_nommu_expand_for_mapping(inode, newsize);
	}
 ...
 */
static int uosfs_nommu_setattr(struct mnt_idmap *idmap,
			       struct dentry *dentry, struct iattr *ia)
{
	...
	if (ia->ia_valid & ATTR_SIZE) {
		...
		if (ia->ia_size != size) {
			ret = uosfs_nommu_resize(inode, ia->ia_size, size);
